...
{
    sub = n.subscribe<yhs_can_msgs::ctrl_cmd>("ctrl_cmd",5,&can_control::ctrl_cmdCallBack,this);
	pub = n.advertise<yhs_can_msgs::ctrl_fb>("ctrl_fb",5);
} */

//将速度写入总线
void can_control::ctrl_cmdCallBack(const yhs_can_msgs::ctrl_cmd::ConstPtr& msg)
{
/*     vel = 0;
    gear=msg->ctrl_cmd_gear;
    //vel=msg->ctrl_cmd_velocity;
	angular = msg->ctrl_cmd_steering * 100;
    brake=msg->ctrl_cmd_Brake;

    static unsigned char count = 0;
	unsigned char sendData_u_vel[8];

    //uint16_t send_frames_[16]; */
...
{
	sub = n.subscribe<yhs_can_msgs::ctrl_cmd>("ctrl_cmd", 5, &can_control::ctrl_cmdCallBack, this);
	//io_cmd_sub_ = nh_.subscribe<yhs_can_msgs::io_cmd>("io_cmd", 5, &can_control::io_cmdCallBack, this);

	//cmd_sub_ = nh_.subscribe<geometry_msgs::Twist>("smoother_cmd_vel", 5, &can_control::cmdCallBack, this);

	//cmd_init_data_sub_ = nh_.subscribe<yhs_can_msgs::ctrl_cmd_init_data>("can_data", 5, &can_control::ctrl_init_data_cmdCallBack, this);
	//cmd_relay_sub_ = nh_.subscribe<std_msgs::UInt8>("can/relay", 5, &can_control::ctrl_relay_cmdCallBack, this);
	...
	boost::thread recvdata_thread(boost::bind(&can_control::recvData, this));
	ros::spin();
	close(dev_handler_);
}
...
{
    ros::init(argc, argv ,"yhs_can_fr07_node");
    can_control cc;
	cc.run();
    //ros::spin();
    return 0;
